...
def login(): #allows the user to login
...
@app.route('/logout')
def logout(): #allows the user to logout
    response = redirect(url_for('login'))
    response.set_cookie('username', '')
    return response

@app.route('/')
def index(): #page that shows payment history of the logged in user
...
def pay(): #method for the user to make payments
    username = request.cookies.get('username')
    if not username:
        return redirect(url_for('login'))
